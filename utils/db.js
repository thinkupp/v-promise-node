        ...
        try {
            if (option.startId === -1) {
                option.startId = 99999999;
					  ...
					  const result = await dbQuery(`select appoint.*, users.nickname, users.avatar, users.gender from (appoint, users, watcher) where watcher.appointId = appoint.id and watcher.userId = ${uid} and users.id = appoint.creatorId and appoint.deleted = 0`);
